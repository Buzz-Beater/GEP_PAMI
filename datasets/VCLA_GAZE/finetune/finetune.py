...
import datasets.VCLA_GAZE.finetune.model as models
import datasets.VCLA_GAZE.finetune.vcla_gaze_finetune as vcla_gaze
...
def validate(data_loader, model, args, test=False, save=False):
    ...
    if test:
        ...
        macro_prec, macro_recall, macro_fscore = utils.compute_accuracy(all_gt_detections, all_detections, metric='macro')
...
def main(args):
    ...
    model = models.TaskNet(task=args.task, name=args.model, feature_dim=feature_dim, obj_feature_dim=obj_feature_dim)
    if not args.distributed:
        model=torch.nn.DataParallel(model).cuda()
        # model.cuda(torch.device(1))
    else:
    ...
    train_dataset = vcla_gaze.VCLA_GAZE_FEATURE(paths, train_ids, transform, input_imsize, 'train', task=args.task)
    val_dataset = vcla_gaze.VCLA_GAZE_FEATURE(paths, val_ids, transform, input_imsize, 'validation', task=args.task)
    test_dataset = vcla_gaze.VCLA_GAZE_FEATURE(paths, test_ids, transform, input_imsize, 'test', task=args.task)
    all_dataset = vcla_gaze.VCLA_GAZE_FEATURE(paths, sequence_ids, transform, input_imsize, 'all', task=args.task)
