            ...
            if fnmatch.fnmatch(filename, '{}_obj*.txt'.format(sequence_id)):
                ...
                with open(os.path.join(eventdir, filename)) as f:
                    ...
                    for line in f:
                        ...
                        pt_count = 0
                        for u in range(image_bbx[0], image_bbx[2], step):
                            for v in range(image_bbx[1], image_bbx[3], step):
                                z = depth[v, u]
                                if z == 0:
                                    continue
                                if not left_handed:
...
def parse_data(paths):
    if os.path.exists(os.path.join(paths.tmp_root, 'activity_corpus.p')):
                ...
                for event in os.listdir(datadir):
                    ...
                    with open(os.path.join(eventdir, 'labeling.txt')) as f:
                        ...
                        for line in f:
                            ...
                            label_list[sequence_id + '$' + str(rel_idx)] = dict()
                            label_list[sequence_id + '$' + str(rel_idx)]['activity'] = metadata.action_index[subactivity]
                            label_list[sequence_id + '$' + str(rel_idx)]['affordance'] = affordance_labels
                            rel_idx += 1
        ...
        pickle.dump(label_list, open(os.path.join(paths.tmp_root, 'label_list.p'), 'wb'))
